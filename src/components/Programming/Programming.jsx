...
function Programming() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const output = await axios.post("https://codecraft2-sak0.onrender.com/run", payload);
            console.log(output);
            const { data } = output;
            setData(data.output);
        } catch (err) {
    ...
    useEffect(() => {
        switch (language) {
            case 'cpp':
                ...
                break;
            case 'py':
                setCode('# Your free instance will spin down with inactivity, which can delay requests/n by 50 seconds or more -render\n # Due to this initial request MAY take time');
                break;
            case 'java':
